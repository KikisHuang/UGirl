...
public class PayActivity extends InitActivity {
    ...
    private void CreateLayouts(int flag, final PayBean pb) {
        ...
        try {
            Glide.with(this).load(pb.getIconUrl()).override(450, 450).into(im);
        } catch (Exception e) {
    ...
    private void createDetailsLayout(LinearLayout pay_details, McVipDescribes mcVipDescribes) {
        ...
        try {
            Glide.with(this).load(mcVipDescribes.getIconUrl()).bitmapTransform(new CropCircleTransformation(this)).override(150, 150).crossFade(200).into(img);
        } catch (Exception e) {
            Log.i(TAG, "Glide You cannot start a load for a destroyed activity");
        }
