...
import example.com.fan.adapter.PhotoPagerAdapter;
 ...
 * 照片查看器页面;
 */
public class PhotoActivity extends InitActivity implements View.OnClickListener, editeListener, CollectListener, PhotoBarListener, TouchCloseListener {
    private static final String TAG = getTAG(PhotoActivity.class);
        ...
        OkHttpUtils
                ...
                .execute(new StringCallback() {
                    ...
                    public void onError(Call call, Exception e, int id) {
                        ToastUtil.toast2_bottom(PhotoActivity.this, "网络不顺畅...");
                    }
    ...
    private void setPage() {
        ...
        viewPager.setAdapter(new PhotoPagerAdapter(getSupportFragmentManager(), urlList, id, pageTopBean));
        ...
        if (urlList.get(0).getMcPublishImgUrls().get(oldposition).getNeedMoney()) {
            ...
            viewPager.setScrollble(MyAppcation.VipFlag, clistener);
        } else
                        ...
                        try {
                            ...
                            if (code == 1) {
                                ...
                                if (ar.length() > 0) {
                                    ...
                                    Glide.with(PhotoActivity.this).load(rb.getImgUrl()).into(bottom_Advertisement_bar);
                                    MzFinal.AdvertisementIsShow = true;
                                    bottom_Advertisement_bar.setOnClickListener(new View.OnClickListener() {
                                        @Override
                                        public void onClick(View v) {
                                            goBannerPage(PhotoActivity.this, rb.getType(), rb.getHttpUrl(), rb.getValue());
                                        }
                                    });
                                } else
                        ...
                        try {
                            ...
                            if (code == 1) {
                                switch (getJsonSring(response)) {
                                    case "1":
                                        ToastUtil.toast2_bottom(PhotoActivity.this, "点赞成功!");
                                        admire_num.setText(ParseK(admire_num.getText().toString(), true));
                                        break;
                                    case "0":
                                        ToastUtil.toast2_bottom(PhotoActivity.this, "取消点赞!");
                    ...
                    public void onResponse(String response, int id) {
                        try {
                            ...
                            if (code == 1) {
                                switch (getJsonSring(response)) {
                                    case "1":
                                        ToastUtil.toast2_bottom(PhotoActivity.this, "收藏成功!");
                                        collect_num.setText(ParseK(collect_num.getText().toString(), true));
                                        break;
                                    case "0":
                                        ToastUtil.toast2_bottom(PhotoActivity.this, "成功取消收藏!");
                    ...
                    public void onResponse(String response, int id) {
                        try {
                            ...
                            if (code == 1) {
                                ...
                                BulletStart();
                                ToastUtil.toast2_bottom(PhotoActivity.this, "评论成功!");
                            } else
                                ToastUtil.ToastErrorMsg(PhotoActivity.this, response, code);
                        } catch (Exception e) {
...
//        MyAppcation.getRefWatcher(this).watch(this);
    ...
    public void ShowVip() {
        if (pay != null && pay.isShowing()) {
            ...
            Log.i(TAG, "popShow");
            PayPopupWindow p = new PayPopupWindow(PhotoActivity.this, String.valueOf(urlList.get(0).getPrice()), id);
            // 一个自定义的布局，作为显示的内容
            View contentView = LayoutInflater.from(PhotoActivity.this).inflate(R.layout.pay_pp_layout, null);
            int width = DeviceUtils.getWindowWidth(PhotoActivity.this) * 8 / 10;
            int h = (int) (DeviceUtils.getWindowHeight(PhotoActivity.this) * 6 / 10);
            pay = new PopupWindow(contentView, width, h);
            p.ScreenPopupWindow(LayoutInflater.from(PhotoActivity.this).inflate(R.layout.photo_activity_layout, null), pay, 0, contentView);
        }
    }
...
//                bottom_Advertisement_bar_layout.setVisibility(View.GONE);
    ...
    public void CloseOfOpenTouch(boolean flag) {
        viewPager.setScrollble(flag, PhotoActivity.clistener);
    }
}
