...
public class VideoController implements View.OnClickListener {
    private static final String TAG = getTAG(VideoController.class);
    ...
    private LinearLayout panorama_ll, panorama_layout;

    /**
     * @param toolbar              底部toolbar;
     * @param player               Inteface;
     * @param changeOrientation    全屏开关;
     * @param video_play_top_tools 顶部toolbar;
     */
    public VideoController(RelativeLayout toolbar, PlayerControl player, boolean changeOrientation, RelativeLayout video_play_top_tools) {
        this.player = player;
        ...
        tbtnGyro = (ToggleButton) video_play_top_tools.findViewById(R.id.video_tool_tbtnGyro);          // 陀螺仪
        tbtnDualScreen = (ToggleButton) video_play_top_tools.findViewById(R.id.video_tool_tbtnVR);      // 单双屏
        panorama_ll = (LinearLayout) video_play_top_tools.findViewById(R.id.panorama_ll);      // 陀螺仪
        panorama_layout = (LinearLayout) video_play_top_tools.findViewById(R.id.panorama_layout);      // 单双屏
        ...
        panorama_ll.setOnClickListener(this);
        panorama_layout.setOnClickListener(this);
        //初始化直接设置全屏;
        if (player != null) {
            player.toFullScreen();
        }
    ...
    public void VideoonResume() {
        player.play();
        tbtnPlayPause.setChecked(false);
    }
    ...
    public void VideoInit(boolean PATTERN) {
    ...
    public void setInfo() {
        ...
        // 设置播放时间
        videoTimeString = PlayerUtils.getShowTime(duration);
                ...
                case 0://更新进度条
                    if (player != null) {
                        ...
                        if (position >= 0 && videoTimeString != null) {
                            ...
                            // 设置播放时间
                            String cur = PlayerUtils.getShowTime(position);
    ...
    public void setPanorama() {
     ...
     /*       player.setDualScreenEnabled(false);
            tbtnDualScreen.setChecked(false);*/
        }
    }
}
