...
public class PhotoPagerAdapter extends FragmentStatePagerAdapter {
    ...
    public PhotoPagerAdapter(FragmentManager fm, List<MirrorBean> urlList, String id) {
        super(fm);
    ...
    public Fragment getItem(int position) {
        ...
        return PictureSlideFragment.newInstance(list.get(position).getPath(), list.get(position).getBasePath(), list.get(position).getNeedMoney(), id, MzFinal.AdvertisementIsShow && position == list.size() - 1 ? true : false);//返回展示不同网络图片的PictureSlideFragment
    }
