    ...
    public static List<McOfficialSellImgUrls> getBuyTopImg(final List<McOfficialSellImgUrls> images, final Context context, LayoutInflater inflater) {
        ...
        for (int i = 0; i < images.size() + 2; i++) {
            ...
            if (i == 0) {   //判断当i=0为该处的ImageView设置最后一张图片作为背景
                ...
                Glide.with(context.getApplicationContext()).load(images.get(images.size() - 1).getPath()).into(imageView);
                ...
                Glide.with(context.getApplicationContext()).load(images.get(0).getPath()).into(imageView);
...
//                imageView = setImg(context);
                Glide.with(context.getApplicationContext()).load(images.get(i - 1).getPath()).into(imageView);
